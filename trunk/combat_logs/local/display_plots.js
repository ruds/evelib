...
var extract_plots = function(you_field, enemy_field) {
  var plots = [];
  $.each(data,
         function(idx, val) {
...
var make_plot = function(selector, you_field, enemy_field, min, max) {
  $.plot($(selector), extract_plots(you_field, enemy_field),
         { xaxis: { mode: 'time',
                    min: min,
                    max: max
                  },
...
$(document).ready(
  function() {
    var min = Math.min.apply(
      null,
      $.map(data, function (val) { return val.start_time; }));
    var max = Math.max.apply(
      null,
      $.map(data, function (val) { return val.end_time; }));
    make_plot('#attack', 'attacker', 'target', min, max);
    make_plot('#defense', 'target', 'attacker', min, max);
  });
